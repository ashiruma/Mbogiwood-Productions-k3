...
export async function POST(request: NextRequest) {
  ...
  if (ResultCode === 0) {
    ...
    try {
      ...
      console.log(
        `✅ Purchase processed for user ${pendingTx.userId}, movie ${pendingTx.movieId}`
      );
    } catch (error: any) {
    ...
    console.log(
      `❌ Payment failed for ${CheckoutRequestID}. Desc: ${ResultDesc}`
    );
    ...
    try {
      ...
      console.error(
        "Error deleting failed pending transaction:",
        error.message
      );
    }
  }
